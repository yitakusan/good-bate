...
GOODS_PATH = re.compile(r"/shop/g/g([^/\"'\s>]+)/?", re.I)
ITEM_PAGE_CONCURRENCY = 6
DEFAULT_LIST_PATH = "shop/default.aspx"
MAX_LIST_PAGES = 30


class FutureshopScraper(Scraper):
    source_platform = "futureshop"
    ...
    async def scrape(self, source: SourceConfig, limit: int | None = None) -> list[RawProduct]:
        ...
        if not item_urls:
            raise RuntimeError(f"未在 {base_url} 找到 FutureShop 商品链接")
            ...
            for list_path in list_paths:
                ...
                for page_num in range(1, MAX_LIST_PAGES + 1):
                    ...
                    try:
                        ...
                        break
                    new_count = 0
                    for goods_id in GOODS_PATH.findall(response.text):
                        item_url = urljoin(base_url, f"shop/g/g{goods_id}/")
                        if item_url in seen:
                            continue
                        seen.add(item_url)
    ...
    def _parse_price(self, text: str, html: str) -> int | None:
        for pattern in [r"([\d,]+)\s*円", r"¥\s*([\d,]+)", r"([\d,]+)\s*\(税込\)"]:
    ...
    def _parse_stock_status(self, text: str, html: str) -> str:
        if any(marker in text for marker in ("SOLD OUT", "売り切れ", "完売", "品切れ")):
            return "缺货"
        if "カートに入れる" in html or "買い物かご" in html:
            return "可购买"
        return "未知"
